...
from simplicity.common.event import EventDeps, EndResult, EventTaskOutput, EventTaskOutputStreamDelta, EventTaskOutputStream
...
class SearchEngine:
    ...
    async def initialize(self):
        ...
        if not self.initialized:
            try:
                ...
                raise RuntimeError(f"Failed to initialize search engine: {str(e)}")
        return True
    ...
    async def search(self, query: str, search_lang: str = "English") -> AsyncGenerator:
        ...
        async for event in event_deps.consume(
            lambda: self.engine.summary_qa(event_deps, query, search_lang)
        ):
            yield event
...
def run_search_with_ui_updates(search_engine, query, search_language, thinking_placeholder, sources_placeholder, result_placeholder):
    ...
    loop_manager.run_async_generator(
        lambda: search_engine.search(query, search_language),
        result_queue
    )
                        ...
                        if not sources_found:
                            ...
                            with sources_placeholder.container():
                                ...
                                if hasattr(event_data.task_output, '__len__'):
                                    st.success(f"Found {len(event_data.task_output)} relevant sources")
                                    
                                    # Show source details in an expander
                                    with st.expander("View Source Details", expanded=False):
                                        for i, source in enumerate(event_data.task_output):
                                            with st.container():
                                                ...
                                                if hasattr(source, 'publishedTime') and source.publishedTime:
                                                    st.markdown(f"- Published: {source.publishedTime}")
...
def main():
    ...
    with st.sidebar:
        ...
        try:
            if st.session_state.search_language in language_options:
                ...
                current_index = 2  # Default to "English" if current language not in predefined list
        except ValueError:
        ...
        if selected_language_option == "Same as query":
            ...
            search_language = st.text_input(
                ...
                value=st.session_state.search_language if st.session_state.search_language not in language_options[2:] else "",
    ...
    if search_button and query.strip():
        ...
        with st.container():
            ...
            try:
                ...
                st.error(f"Failed to execute search: {str(e)}")
                st.exception(e)
    ...
    if not st.session_state.current_search:
        ...
        st.markdown("Using your native language to query and answer, but search in another language")
        col1, col2, col3 = st.columns(3)
        
        with col1:
            if st.button("🗾 (日) Working visa detailed requirements in Japan", use_container_width=True):
                st.session_state.current_search = "Working visa detailed requirements in Japan"
        ...
        with col3:
            if st.button("🌍 (中) How to take subway in Shanghai", use_container_width=True):
