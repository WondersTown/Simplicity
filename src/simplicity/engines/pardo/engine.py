...
async def _single_qa(engine: PardoEngine, deps: BaseDeps, query: str, source: str) -> str:
    SYSTEM_PROMPT = """
...
{query}
    ...
    agent = Agent(
        model=engine.single_qa_llm.model,
        model_settings=engine.single_qa_llm.settings,
...
async def _translate(engine: PardoEngine, deps: BaseDeps, query: str, lang: str) -> str:
    agent = Agent(
        model=engine.translate_llm.model,
        model_settings=engine.translate_llm.settings,
...
async def summary_qa(engine: PardoEngine, deps: BaseDeps | None, query: str, search_lang: str | None = "English"):
    deps = deps or BaseDeps()
    if search_lang is not None:
        query_search = await _translate(engine, fork_deps(deps), query, search_lang)
    else:
        query_search = query
    searched = await search_with_read(engine.jina_search_client, engine.jina_read_client, query_search, num=9, timeout=15)
...
"""
    answers = await instrument(gather)(
        *[
            _single_qa(engine, fork_deps(deps), query, x.content)
            for x in searched
        ],
    )
    answers = [
    ...
    agent = Agent(
        model=engine.summary_qa_llm.model,
        model_settings=engine.summary_qa_llm.settings,
        system_prompt=SYSTEM_PROMPT,
        deps_type=BaseDeps
    )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        async for event in summary_qa(engine, None, "日本的首都是哪里?", "English"):
