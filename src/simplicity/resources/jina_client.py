...
class JinaSearchClient:
    ...
    async def search(self, query: str, page: int = 1) -> SearchResponse:
        response = await self.client.get(url="/", params={"q": query, "page": page})
...
class JinaReadClient:
    ...
    async def read(self, target_url: str, timeout: int = 15) -> ReaderResponse:
        ...
        response = await self.client.get(
            url=f"/{target_url}", headers=headers, timeout=timeout + 10
        )
...
async def search_with_read(
    ...
    try:
        ...
        while len(search_l) < num:
            search_l.extend((await search_cli.search(query, page)).data)
            page += 1
